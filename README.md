...
- `weather_fetch.py`: 고도 보정 공식은 단순화된 버전입니다. `STATION_ELEV`에 없는 관측소를 쓸 경우 기본값(50m)이 적용되므로 필요 시 직접 추가하세요. 또한 기상청 API 응답의 기온(`ta`)·습도(`hm`) 컬럼 인덱스는 `help=1` 옵션으로 실제 헤더를 확인 후 조정이 필요할 수 있습니다.
